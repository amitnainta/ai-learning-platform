...
Recorded during a subsequent fix pass addressing REVIEW.md's one blocking finding (B1). No plan tasks were reopened or expanded; this is a correction to already-checked tasks 9 and 30.

1. **B1 — re-flagging a rating whose earlier flag from the same reporter was already resolved silently never reached the moderation queue — fixed.** `flagRating` (`src/lib/ratings/mutations.ts`) upserts on `ratingId_userId` (decision #5), so a repeat report by the same user always takes the `update` branch. That branch only ever touched `reason`/`note`, never resetting `resolvedAt`, so once `hideRating`/`dismissRatingFlags` had stamped `resolvedAt` on a flag, every later report the same user filed against that rating wrote a row `listUnresolvedFlagsGroupedByRating`'s `resolvedAt: null` filter excluded forever — while `POST /api/ratings/flags` still returned `{ success: true }` and `<FlagRatingButton>` still announced "Reported — thank you." Fixed by adding `resolvedAt: null` to the `update` clause, so a repeat report genuinely reopens the queue entry, matching decision #5's own stated intent that a flag needs "a real, non-dead-end action loop."
2. Test coverage added at both layers: `src/lib/ratings/__tests__/mutations.test.ts`'s existing `flagRating` case (which previously asserted the defective `update` shape — `{ reason: "SPAM", note: null }` with no `resolvedAt` — as correct) now asserts `{ reason: "SPAM", note: null, resolvedAt: null }`, and a second case explicitly asserts the `update` clause carries `resolvedAt: null` on a differently-shaped call. `src/lib/ratings/__tests__/flag-lifecycle.test.ts` (new) is an end-to-end regression test against a small in-memory fake of the `ratingFlag` table (not per-call mock assertions): it flags a rating, confirms it appears in `listUnresolvedFlagsGroupedByRating`'s output, dismisses it via `dismissRatingFlags` and confirms the queue empties, then re-flags the same rating from the same reporter and confirms the queue entry reappears with the new reason — proving the fix reaches the moderation queue in practice, not just that the upsert call shape looks right.
3. Full toolchain re-run fresh after the fix: `npm ci`, `npm run lint`, `npm run format:check`, `npx tsc --noEmit`, `npm run build`, `npm run test` (347 tests, up from 345 — the two new B1 cases — all passing; one unrelated `src/lib/auth/__tests__/options.test.ts` timeout on the first full-suite run was confirmed as parallel-run flakiness by an isolated re-run and by a clean second full-suite run), and `npm run test:e2e` against a real local Postgres (all three ratings specs — `rating-course.spec.ts`, `rating-flag.spec.ts`, `rating-path.spec.ts` — green on both runs; the one failure seen, `account-profile.spec.ts`, is a pre-existing spec this fix pass never touches and was confirmed as 8-worker parallel-load flakiness by an isolated re-run that passed cleanly).
