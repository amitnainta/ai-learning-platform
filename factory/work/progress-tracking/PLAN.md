...
## Builder notes (fix pass — REVIEW.md B1)

Recorded during a subsequent fix pass addressing REVIEW.md's one blocking finding (B1). No plan tasks were reopened or expanded; this is a correction to already-checked task 20.

1. **B1 — dashboard dead end for a role/level with no published path — fixed.** `src/app/(app)/dashboard/page.tsx` computed `hasPath` from whether `user.roleArchetype`/`user.level` were merely *set* (true for `ENGINEERING_LEADER`/`INVESTOR`, two of the five selectable roles), not from whether `getPathForRoleLevel()` actually *resolved* a path for that combination — content/paths/ only has `technical-builder-*` and `executive-non-technical-*`. A user onboarded as Engineering Leader or Investor therefore had `hasPath === true` but `activePaths` empty, so both the path cards and the "no path available, browse paths" fallback were skipped: greeting, role/level list, path switcher, nothing else. Fixed by having `getDashboardProgress` (`src/lib/progress/queries.ts`) return a new `hasCurrentPath: boolean` field set from `currentPathDetail !== null` — i.e. whether `getPathForRoleLevel` actually resolved a path, not whether the fields are populated — and having the dashboard page branch the fallback on `roleOrLevelUnset || (!hasCurrentPath && activePaths.length === 0)`. This preserves all three real cases: (a) no role/level or `NOT_SURE_YET` → unchanged onboarding-prompt behaviour; (b) role/level set and a path exists → path cards render, fallback never shows; (c) role/level set but no path exists for that combination → the fallback shows *unless* the user has previously-followed paths with progress (decision #7), in which case those retained-path cards render instead of a dead end and the fallback is correctly suppressed.
2. Test coverage added at both layers: `src/lib/progress/__tests__/queries.test.ts` gained two cases asserting `hasCurrentPath: false` for an `ENGINEERING_LEADER` user with no published path — one with no prior progress (`activePaths` empty, the previously-broken case), one with progress on a previously-followed Technical Builder path (`activePaths` has one non-current entry). `e2e/progress-dashboard.spec.ts` gained a `describe` block with two real-browser cases: registering and onboarding as `ENGINEERING_LEADER` with no prior progress asserts the "Browse learning paths" heading, the "No path is available for your current role and level yet." copy, and the "Browse paths" link are all visible (and that no path region renders); the second registers as `TECHNICAL_BUILDER`, completes an item, then switches role to `ENGINEERING_LEADER` via `PATCH /api/account/profile` and asserts the retained Technical Builder path card is still visible on `/dashboard` with the fallback heading absent.
